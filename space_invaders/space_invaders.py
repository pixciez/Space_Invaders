...
def create_ui_elements():
    ...
    win_alien = turtle.Turtle()
    win_alien.shape("img/alien.gif")
    win_alien.penup()
    win_alien.goto(0, 100)
    win_alien.hideturtle()

    return score_pen, game_over_text, high_score_display, win_text, win_alien
...
def all_elis_destroyed():
    return all(not eli.isvisible() for eli in elis)

def show_win_message():
    global game_state
    game_state = "won"
    wn.bgcolor("gray20")  # Dim the background
    win_text.clear()
    win_text.write("YOU WIN!", align="center", font=("Press Start 2P", 30, "normal"))
    win_alien.showturtle()
    win_alien.goto(-5, 115)  # Move the alien down a bit
    show_play_again_button()

def handle_win_state():
    global elispeed
    elispeed = 0  # Stop the elis from moving
    bullet.hideturtle()  # Hide the bullet

def show_play_again_button():
    play_again_button.place(relx=0.5, rely=0.6, anchor=tk.CENTER)
...
def reset_elis():
    global elis, elispeed
    for eli in elis:
        eli.hideturtle()
    elis = create_elis(30)
    elispeed = 0.02  # Reset speed to initial value
...
score_pen, game_over_text, high_score_display, win_text, win_alien = create_ui_elements()
